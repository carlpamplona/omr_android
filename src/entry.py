...
cnx = mysql.connector.connect(user='root', password='',
                              host='localhost', database='db_exams')
cursor = cnx.cursor()
    ...
    for file_path in omr_files:
        ...
        if (
            ...
            for question, answer in omr_response.items():
                if question not in ['Roll', 'ExamCode']:
                    if answer.strip() != '':
                        query = f"INSERT INTO answers (student_no, exam_id, question, answer) VALUES ('{omr_response['Roll']}', '{int(omr_response['ExamCode'], 2)}', '{question[1:]}', '{answer}') ON DUPLICATE KEY UPDATE answer='{answer}'"
